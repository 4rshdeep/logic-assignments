...
let rec sublist b e l = 
  match l with
    [] -> failwith "sublist cannot be made b has to less than e"
  | h :: t -> 
...
let combs_with_rep m xs =
  ...
  arr.(0) <- [[]];
  List.iter (fun x ->  for i = 1 to m do
...
let combs i l =
    ( 
        let n = List.length l in 
            if n>i then (combs_with_rep i l) else 
...
let rec get_func f = match f with 
        F(x, y)::xs -> F(x, y) :: (get_func xs)
    |   V(x):: xs   -> (get_func xs)
    |   C(x) :: xs  -> (get_func xs)
    |   [] -> []
;;

let rec get_terms l x  = match x with 
            PRED (x, y)     -> (get_func y)@l
        |   NOT f           -> (get_terms l f)
        |   AND (a, b)      -> (union (get_terms l a) (get_terms l b)) 
        |   OR (a, b)       -> (union (get_terms l a) (get_terms l b)) 
;;

let make_constant x = C x;;
                                            ...
                                            let f = replace_constants cts c1 f in 
                                                ...
                                                let vars_assignment = (combs (List.length vrs) c2) in
                                                    let k = dppll_helper vrs vars_assignment f in 
                                                        if fst k then (((List.map make_constant domain)@(make_list(List.map (get_terms []) (thd k)))) , thd(k)) else (raise (DPLL_unsat y)) 
                                else 
...
(*  helper dpll function used by sat for iterative model finding from i = 1 to some k *)
let dpll_h x y = 
    (
        let f = scnf x in
            let cts = consts f in
                                    ...
                                    let c1 = sublist 0 (l-1) domain in 
                                        let c2 = domain in 
                                            let f = replace_constants cts c1 f in 
                                                let vars_assignment = (combs (List.length vrs) c2) in
                                                    let k = dppll_helper vrs vars_assignment f in 
                                                        if fst k then (true, ((List.map make_constant domain)@(make_list(List.map (get_terms []) (thd k)))) , thd(k)) else (false , [], []) 
                                else 
                                    let vars_assignment = (combs (List.length vrs) domain) in
                                        let k = dppll_helper vrs vars_assignment f in 
                                            if fst k then (true, ((List.map make_constant domain)@(make_list(List.map (get_terms []) (thd k)))) , thd(k)) else (false, [], []) 
...
let rec sat_helper f i k = 
    if i <= k then 
        let b = dpll_h f i in 
            if (fst b) then b else sat_helper f (i+1) k
    else 
        raise (DPLL_unsat(k))
;;
